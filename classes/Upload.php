                ...
                if ($id) {
                    ...
                    foreach ($signatarios as $email) {
                        ...
                        if (!empty($email_limpo) && filter_var($email_limpo, FILTER_VALIDATE_EMAIL)) {
                            if ($doc->adicionarSignatario($id, 'Signatário', $email_limpo)) {
                    ...
                    $msg .= 'Verifique as permissões do banco.';
                    $tipo_msg = 'danger';
                }
            } else {
                $msg = $upload['message'];
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select id="projeto_id" name="projeto_id" class="form-control">
                ...
                <?php foreach ($projetos as $p): ?>
                    <option value="<?php echo $p['id']; ?>" <?php echo (($_POST['projeto_id'] ?? '') == $p['id']) ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <div id="signatarios-container">
                <?php if (isset($_POST['signatarios'])): ?>
                    <?php foreach ($_POST['signatarios'] as $email): ?>
                        <input type="email" name="signatarios[]" class="form-control mb-2" placeholder="E-mail do signatário" required value="<?php echo htmlspecialchars($email); ?>">
                    <?php endforeach; ?>
                <?php else: ?>
